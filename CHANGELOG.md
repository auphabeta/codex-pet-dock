...
- Theme Studio 可以打开已有主题，采用原子写入保存，并通过命名事件通知主程序
  热重载，无需重启 Pet Dock。
- 主题菜单增加 `Create with Codex`、高级手工微调与
...
- 新增随安装包分发的 `$codex-pet-dock-theme` Skill 与
  ...
  普通用户只需描述风格，不再从 Theme Studio 参数表开始。
...
- 底座承托面与宠物脚部重叠时不再拦截宠物拖动；顶部装饰区域执行鼠标命中穿透，
  下方指标区域仍可点击展开详情，并加入真实窗口回归测试。
...
- 额度自动刷新保持每 5 分钟一次；打开陈旧详情时按需刷新，失败退避与正常
  刷新解耦，按 15、30、60 分钟逐级延长，避免断网或 Codex 不可用时重复拉起
  探测进程。
