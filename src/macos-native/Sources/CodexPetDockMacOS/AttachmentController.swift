...
final class AttachmentController {
    ...
    init(overlay: DockOverlayController) {
        ...
        overlay.onDragEnded = { [weak self] _ in
    ...
    private func tick() {
        ...
        overlay.show(
            ...
            allowsDragging: !precise
        )

        if precise {
            publishStatus("Attached to Codex pet — move the pet; dock follows")
        } else {
    ...
    private func handleDrag(_ delta: CGPoint) {
        guard previewEnabled || currentAnchor?.source == .hostWindowFallback else {
            return
        }
