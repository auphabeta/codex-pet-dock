...
候选在静止时最多每 750ms、拖动时最多每 125ms 重新发现一次，不长期缓存旧宠物。因此切换宠物、通知
展开/收起或内部 DOM 重新创建后，底座会绑定新元素。

UI Automation 暂时失败时，最后一次有效宠物边界保留 2 秒，避免切换动画
期间闪烁；持续失败则回退到宿主窗口锚点或安全隐藏。
...
- 快速模式在移动停止后保留 850ms；
- 完整 UI Automation 后代树扫描在空闲时最多每 750ms 一次，拖动时
  最多每 125ms 一次；其余帧只读取缓存元素边界；
...
`src/quota-probe.mjs` 启动 Codex 自带的可执行文件：
...
自动刷新默认每 5 分钟一次。打开详情且上次查询超过 30 秒，或用户点击
...
- 原型仍由 PowerShell + Node.js 组成，不适合直接称为免依赖成品。
