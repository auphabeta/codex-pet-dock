...
当前已提供面向普通用户的 `Setup.exe`，实现每用户安装、覆盖升级、开始菜单、
登录自启和完整卸载。常驻界面仍由 PowerShell + WinForms 承载，因此当前定位
是可安装的 Windows Beta；完成单文件迁移、代码签名和多机器验证后再进入 1.0。
...
- 主题选择持久化；
- 无需管理员权限的运行模型；
- Inno Setup 每用户安装器、升级保留和卸载入口；
- 便携 ZIP、Setup 与 SHA-256 自动构建；
- 明确的失败安全和隐私边界。
...
- 不要求管理员权限；
- 默认安装到当前用户目录，也允许选择 `D:\CodexPetDock` 这类其他本地磁盘目录；
- 拒绝把盘符根目录或网络路径作为安装目录，升级时不会清空整个自定义目录；
- 创建开始菜单快捷方式；
- 安装结束可选择立即启动；
- 登录自启默认开启，并在首次提示与托盘中提供清楚、可逆的关闭入口；
- 升级覆盖程序文件但保留配置；
- 卸载默认保留用户配置，用户可手动删除配置目录。
...
当前版本已经达到“普通用户可双击安装的 Windows Beta”，但仍不应宣传为
经过签名与大规模验证的 1.0。完成 .NET 单文件迁移、Authenticode 签名和
至少一轮多机器兼容测试后，即可进入稳定版阶段。本项目不依赖 CDP 注入，
升级风险和权限面更小。
